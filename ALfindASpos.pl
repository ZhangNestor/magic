...
while(<A3>){
  ...
  if($arrloc[6]=~m/\+/){
    ...
    @arrloc2=split /\-/,$arrloc[4];    
    $Withfirst=$arrloc1[1]-1;
    $Wiofirst=$arrloc2[1]-1;
  }
  if($arrloc[6]=~m/\-/){
    ...
    foreach $id (keys %hasht) {
      ...
      if($arrid1[0]=~m/$arrid_withA3[$i]/){
        ...
        for($m=0;$m<scalar(@arrexon);$m++){
          if($arrexon[$m]=~m/$Withfirst/){
            print OUT "WithAL","\t",$arrid1[1],"\t",$arrid_withA3[$i],"\t",$arrloc[6],"\t",$arrexon[$m],"\t",$hasht{$id},"\n"; 
          }
        }
      }      
    }
  ...
  for($i=0;$i<scalar(@arrid_wioA3);$i++){
    foreach $id (keys %hasht) {
      ...
      if($arrid1[0]=~m/$arrid_wioA3[$i]/){
        @arrexon=split /\#/,$hasht{$id};
        for($m=0;$m<scalar(@arrexon);$m++){
          if($arrexon[$m]=~m/$Wiofirst/){
            print OUT "WioAL","\t",$arrid1[1],"\t",$arrid_wioA3[$i],"\t",$arrloc[6],"\t",$arrexon[$m],"\t",$hasht{$id},"\n";   
          }
        }
      }      
    }
